...
import { loadConfig, savePresets, parseCommandLineArgs } from './utils';
...
async function main() {
  try {
    ...
    if (!args.interactive && args.mode) {
      console.log(`Running in non-interactive mode: ${args.mode}`);

      if (args.mode === 'backup' && args.source) {
        const backupMode = args.backupMode || 'all';
        const collections = args.collections || [];
        await runBackup(args.source, backupMode, collections);
        return;
      }

      if (args.mode === 'restore' && args.backupFile && args.target) {
        await runRestore(args.backupFile, args.target, []);
        return;
      }
...
async function backupDatabase() {
  ...
  const { source, selectedCollections, excludedCollections } =
    await promptService.promptForBackup();
...
async function managePresets() {
  ...
  console.log(
    `DEBUG: Config contains ${config.backupPresets?.length || 0} backup presets and ${config.restorePresets?.length || 0} restore presets`
  );
  if (config.backupPresets) {
  ...
  // Check for presets
  if (
    (!config.backupPresets || config.backupPresets.length === 0) &&
